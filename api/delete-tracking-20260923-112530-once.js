...
const TARGET_RECORDED='2026-09-23 11:25:30';
const TARGET_APPOINTMENT_DATE='2026-10-29';
const TARGET_APPOINTMENT_TIME='10.15';
const TARGET_ROUTE='PKV';
const TARGET_PATIENT='existing';
const TARGET_SERVICE='Vorsorge';
const TARGET_DOCTOR='Dr. med. Christina Moxter';
...
function normTime(v){return String(v||'').replace(':','.')}
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const matches=records.filter(({event})=>
      berlinStamp(event?.recordedAt)===TARGET_RECORDED &&
      String(event?.appointmentDate||'')===TARGET_APPOINTMENT_DATE &&
      normTime(event?.appointmentTime)===TARGET_APPOINTMENT_TIME &&
      String(event?.route||'')===TARGET_ROUTE &&
      String(event?.patientType||'')===TARGET_PATIENT &&
      String(event?.serviceName||'')===TARGET_SERVICE &&
      String(event?.doctorName||'')===TARGET_DOCTOR &&
      Number(event?.duration)===TARGET_DURATION
    );
    if(matches.length!==1){
    ...
    return send(res,200,{
      ...
      route:target.event.route,
      patientType:target.event.patientType,
      serviceName:target.event.serviceName,
      doctorName:target.event.doctorName,
      duration:target.event.duration
    });
  }catch(e){
