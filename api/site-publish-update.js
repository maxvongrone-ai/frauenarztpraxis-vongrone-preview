...
const TOKEN_SHA256='761aad34859490808aaf7d7708c64dff833dc3a22122ccfb9cf25a9fea0ffa7b';
...
const FILES=[
  ...
  {src:'sitemap.xml',dst:'/frauenarztpraxis/sitemap.xml',must:['https://frauenarztpraxis-vongrone.de/schwangerschaft/','https://frauenarztpraxis-vongrone.de/wechseljahre/']}
];
...
module.exports=async function handler(req,res){
  if(req.method==='GET'){
    ...
    res.setHeader('Pragma','no-cache');
    res.setHeader('X-Robots-Tag','noindex, nofollow, noarchive');
    return res.end(`<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Praxiswebsite aktualisieren</title><style>body{font-family:system-ui,-apple-system,sans-serif;max-width:720px;margin:56px auto;padding:0 20px;color:#303536}h1{font-size:28px}input,button{font:inherit;padding:12px 14px;border-radius:10px;box-sizing:border-box}input{border:1px solid #d9d2ce;width:min(430px,100%)}button{border:0;background:#765f56;color:#fff;margin-top:12px;cursor:pointer}button:disabled{opacity:.55}pre{white-space:pre-wrap;background:#f6f3f1;padding:14px;border-radius:10px;min-height:30px}.ok{color:#18733b}.bad{color:#a33a3a}</style></head><body><h1>Aktualisierte Praxiswebsite übertragen</h1><p>Übertragen werden die aktuelle Startseite, die SEO-Fachseiten einschließlich <strong>Schwangerschaft</strong>, die Kurz-URL <strong>/schwanger/</strong>, <code>robots.txt</code> und <code>sitemap.xml</code>.</p><p>Bitte das aktuell gültige <strong>SFTP-Passwort von United Domains</strong> eingeben. Es wird nur für diese Übertragung verwendet und nicht gespeichert.</p><input id="pw" type="password" autocomplete="off" placeholder="SFTP-Passwort"><br><button id="go">Übertragung starten</button><p id="s"></p><pre id="out"></pre><script>const p=new URLSearchParams(location.hash.slice(1));go.onclick=async()=>{const password=pw.value;if(!password){s.className='bad';s.textContent='Bitte zuerst das SFTP-Passwort eingeben.';return}go.disabled=true;s.className='';s.textContent='Übertragung läuft …';out.textContent='';try{const r=await fetch(location.pathname,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:p.get('token')||'',pw:password})});const j=await r.json();out.textContent=JSON.stringify(j,null,2);s.className=j.ok?'ok':'bad';s.textContent=j.ok?'Erfolgreich übertragen.':'Fehler bei der Übertragung.'}catch(e){s.className='bad';s.textContent='Fehler.';out.textContent=String(e)}finally{go.disabled=false;pw.value=''}}</script></body></html>`);
  }
  ...
  for(const f of FILES){
    const r=await fetch(BASE+f.src,{headers:{'User-Agent':'Praxis-Website-Publisher/1.0'},cache:'no-store'});
    if(!r.ok)return send(res,502,{ok:false,error:'Quelle konnte nicht geladen werden: '+f.src+' (HTTP '+r.status+')'});
    const txt=await r.text();
    if(!f.must.every(x=>txt.includes(x)))return send(res,400,{ok:false,error:'Sicherheitsprüfung fehlgeschlagen: '+f.src});
    loaded.push({...f,text:txt});
  }
  ...
  try{
    ...
    return send(res,200,{ok:true,uploaded,websiteVersion:'2026-09-22-pregnancy'});
  }catch(e){
